...
import { CreditCard, Banknote, Users, UserPlus } from 'lucide-react';
...
export default function PaymentModal({ isOpen, onClose, total, orders, onConfirm }: PaymentModalProps) {
  ...
  return (
    <>
      {/* Dialog uses Radix portal — always renders above all stacking contexts */}
      <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-md max-h-[90vh] overflow-y-auto p-4 md:p-6">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-orange-500">Método de Pago</DialogTitle>
          </DialogHeader>
            ...
            {method === 'mixed' && (
              <div className="space-y-3 bg-slate-700/50 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => setCashPayers(Math.max(0, cashPayers - 1))} className="w-8 h-8 bg-slate-600 hover:bg-slate-500 rounded text-white font-bold text-sm">-</button>
                    <input type="number" min="0" max={splitBetween} value={cashPayers} onChange={(e) => setCashPayers(Math.max(0, Math.min(splitBetween, parseInt(e.target.value) || 0)))} className="flex-1 bg-slate-600 text-white text-center py-1 rounded" />
                    <button onClick={() => setCashPayers(Math.min(splitBetween, cashPayers + 1))} className="w-8 h-8 bg-slate-600 hover:bg-slate-500 rounded text-white font-bold text-sm">+</button>
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-slate-400 text-xs">Pagan con Tarjeta</label>
                  <div className="flex items-center gap-3">
                    <button onClick={() => setCardPayers(Math.max(0, cardPayers - 1))} className="w-8 h-8 bg-slate-600 hover:bg-slate-500 rounded text-white font-bold text-sm">-</button>
                    <input type="number" min="0" max={splitBetween} value={cardPayers} onChange={(e) => setCardPayers(Math.max(0, Math.min(splitBetween, parseInt(e.target.value) || 0)))} className="flex-1 bg-slate-600 text-white text-center py-1 rounded" />
                    <button onClick={() => setCardPayers(Math.min(splitBetween, cardPayers + 1))} className="w-8 h-8 bg-slate-600 hover:bg-slate-500 rounded text-white font-bold text-sm">+</button>
                  </div>
                </div>
            ...
            {((method === 'cash' || method === 'mixed') && cashAmount > 0) && (
              <div className="bg-green-500/10 border-2 border-green-500/30 p-4 rounded-lg space-y-3">
                ...
                <input
                  ...
                  className="w-full bg-slate-700 text-white text-center text-xl font-bold py-2 rounded-lg border border-green-500/30 focus:outline-none focus:border-green-500"
                />
      ...
      {/* Custom Split Modal — also uses Dialog portal */}
      <CustomSplitModal
